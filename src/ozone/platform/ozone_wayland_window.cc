...
void CreateBitmapFromPng(
  ...
  base::MessageLoopCurrentForUI::Get()->task_runner()->PostTask(
...
void OzoneWaylandWindow::InitPlatformWindow(
  ...
  sender_->Send(
      new WaylandDisplay_InitWindow(handle_,
                                    parent_,
                                    bounds_,
                                    type_,
                                    surface_id_));
...
#if defined(OS_WEBOS)
  else if (cursor_type_ == app_runtime::CustomCursorType::kBlank) {
...
void OzoneWaylandWindow::SetCustomCursor(app_runtime::CustomCursorType type,
  ...
  if (allowed_cursor_overriding_ && !allowed_cursor_overriding)
    return;
  if (type != app_runtime::CustomCursorType::kPath && type == cursor_type_ &&
