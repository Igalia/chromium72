...
std::unique_ptr<corewm::Tooltip> DesktopWindowTreeHostOzone::CreateTooltip() {
...
void DesktopWindowTreeHostOzone::Show(ui::WindowShowState show_state,
                                      const gfx::Rect& restore_bounds) {
  if (show_state == ui::SHOW_STATE_MAXIMIZED && !restore_bounds.IsEmpty())
    platform_window_->SetRestoredBoundsInPixels(ToPixelRect(restore_bounds));
  ...
  if (!restore_bounds.IsEmpty())
    previous_bounds_ = restore_bounds;
...
void DesktopWindowTreeHostOzone::ShowWindowWithState(
  ...
  Show(show_state, previous_bounds_);
}
...
void DesktopWindowTreeHostOzone::SetAspectRatio(
    const gfx::SizeF& aspect_ratio) {
  DCHECK(!aspect_ratio.IsEmpty());
  if (content_window_) {
    content_window_->SetProperty(aura::client::kAspectRatio,
...
void DesktopWindowTreeHostOzone::SetCursorNative(gfx::NativeCursor cursor) {
  ...
  if (native_type == ui::CursorType::kNone) {
    ...
    return;
  } else if (native_type != ui::CursorType::kCustom) {
...
void DesktopWindowTreeHostOzone::SetCustomCursor(
    app_runtime::CustomCursorType type,
    const std::string& path,
    int hotspot_x,
    int hotspot_y) {
  // Call SetCustomCursor with allowed_cursor_overriding == true, to override
  // cursor, so it will no longer be changed by css of the corresponding
  // element.
...
void DesktopWindowTreeHostOzone::OnCursorVisibilityChanged(bool visibility) {
#if defined(USE_NEVA_APPRUNTIME)
  views::NativeEventDelegate* native_event_delegate =
      desktop_native_widget_aura_->GetNativeEventDelegate();

  if (native_event_delegate)
    native_event_delegate->CursorVisibilityChanged(visibility);
#endif
}
...
void DesktopWindowTreeHostOzone::OnWindowHostStateChanged(ui::WidgetState new_state) {
  ...
  if (native_event_delegate) {
    ...
    VLOG(1) << __PRETTY_FUNCTION__ << ": state=" << (uint32_t)new_state;
  } else {
    VLOG(1) << __PRETTY_FUNCTION__ << ": state=" << (uint32_t)new_state <<
...
void DesktopWindowTreeHostOzone::OnWindowHostStateAboutToChange(ui::WidgetState state) {
  ...
  if (native_event_delegate) {
    ...
    VLOG(1) << __PRETTY_FUNCTION__ << ": state=" << (uint32_t)state;
  } else {
    VLOG(1) << __PRETTY_FUNCTION__ << ": state=" << (uint32_t)state <<
        " but no native event delegate";
  }
