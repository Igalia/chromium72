...
void WebAppWindowBase::CreateGroup(const WindowGroupConfiguration& config) {
  DCHECK(webapp_window_);
  webapp_window_->CreateGroup(config);
}

void WebAppWindowBase::AttachToGroup(const std::string& group,
                                     const std::string& layer) {
  DCHECK(webapp_window_);
  webapp_window_->AttachToGroup(group, layer);
}

void WebAppWindowBase::FocusGroupOwner() {
  DCHECK(webapp_window_);
  webapp_window_->FocusGroupOwner();
}

void WebAppWindowBase::FocusGroupLayer() {
  DCHECK(webapp_window_);
  webapp_window_->FocusGroupLayer();
}

void WebAppWindowBase::DetachGroup() {
  DCHECK(webapp_window_);
